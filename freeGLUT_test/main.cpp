...
void customShape(float a, float b, float c, float d){
    ...
    for(int i=0; i<nVertex; i++){
        for(int j=0; j<3; j++){
            float temp = 0;
            for (int k=0; k<3; k++){
                temp = temp + trans[j][k]*shownVertices[i][k];
            }
            shownVertices[i][j] = temp;
        }
    }
}
